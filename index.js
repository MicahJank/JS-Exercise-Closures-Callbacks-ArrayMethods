...
*/
function processLastItem(strArray, callback) {
  return callback(strArray[strArray.length - 1]);
}
 ...
 * should return 1000.
*/
function processSum(numArray, callback) {
  const reducedArray = numArray.reduce((accumulator, currentValue) => {
    return accumulator + currentValue;
  }, 0);

  return callback(reducedArray);
}
...
function processContains(item, list, callback) {
    const itemInList = list.includes(item);
    
    return callback(itemInList);
}
...
function processDuplicateFree(list, callback) {
    const updatedList = list.filter((currentValue, index) => {
...
function getFullNames(runners) {
    ...
    runners.forEach(runner => {
      namesArray.push(`${runner.last_name}, ${runner.first_name}`);
    });
